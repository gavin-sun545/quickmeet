...
const transClient = new TranslateClient({ region: "us-east-1" });
...
io.on('connect', socket => {
    ...
    socket.on('translate-send', (srcLang, dstLang, text) => {
        ...
        const command = new TranslateTextCommand(translate_params);
        transClient.send(command).then(
            (data) => {
                // process data.
                socket.emit('translate-result', data.TranslatedText);
            },
            (error) => {
                // error handling.
                console.log(error)
